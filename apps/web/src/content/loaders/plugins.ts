...
export function pluginsLoader(): Loader {
    return {
        name: 'jellyfindex-plugins-loader',
        load: async ({
            config,
            store,
            parseData,
            ...
            const pluginsDir = fileURLToPath(new URL('content/plugins/', repoRoot));
            if (!existsSync(pluginsDir)) {
                logger.warn(`No plugins directory found at ${pluginsDir}`);
                return;
            }

            store.clear();

            const slugs = readdirSync(pluginsDir).filter((name) =>
                statSync(join(pluginsDir, name)).isDirectory()
            );

            for (const slug of slugs) {
                const entryDir = join(pluginsDir, slug);
